...
const sqlPath = path.join(rootDir, "supabase/tests/foundation_report_package_command_smoke.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(finalResult?.rows?.[0]?.result || "foundation report package command smoke completed");
  } finally {
    await client.end();
  }
}
