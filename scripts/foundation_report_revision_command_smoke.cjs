...
const sqlPath = path.join(rootDir, "supabase/tests/foundation_report_revision_command_smoke.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(finalResult?.rows?.[0]?.result || "foundation report revision command smoke completed");
  } finally {
    await client.end();
  }
}
