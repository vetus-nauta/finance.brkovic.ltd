...
const sqlPath = path.join(rootDir, "supabase/tests/foundation_report_snapshot_command_smoke.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(finalResult?.rows?.[0]?.result || "foundation report snapshot command smoke completed");
  } finally {
    await client.end();
  }
}
