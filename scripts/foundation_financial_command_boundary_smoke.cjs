...
const sqlPath = path.join(rootDir, "supabase/tests/foundation_financial_command_boundary_smoke.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(finalResult?.rows?.[0]?.result || "foundation financial command boundary smoke completed");
  } finally {
    await client.end();
  }
}
