...
const sqlPath = path.join(rootDir, "supabase/tests/foundation_report_locked_correction_command_smoke.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(finalResult?.rows?.[0]?.result || "foundation report locked correction command smoke completed");
  } finally {
    await client.end();
  }
}
