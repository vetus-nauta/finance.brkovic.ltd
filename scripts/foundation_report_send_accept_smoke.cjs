...
const sqlPath = path.join(rootDir, "supabase/tests/foundation_report_send_accept_smoke.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(finalResult?.rows?.[0]?.result || "foundation report send accept smoke completed");
  } finally {
    await client.end();
  }
}
