...
const sqlPath = path.join(rootDir, "supabase/tests/foundation_approval_audit_boundary_smoke.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(finalResult?.rows?.[0]?.result || "foundation approval audit boundary smoke completed");
  } finally {
    await client.end();
  }
}
