...
const sqlPath = path.join(rootDir, "supabase/tests/foundation_report_export_version_smoke.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(finalResult?.rows?.[0]?.result || "foundation report export version smoke completed");
  } finally {
    await client.end();
  }
}
