...
TMP_DIR="$(mktemp -d /tmp/findesk-v2-manual-walkthrough.XXXXXX)"
...
DB_NAME="findesk_v2_manual_walkthrough"
COOKIE_NAME="findesk_v2_manual_walkthrough_session"
TOKEN="findesk-v2-manual-walkthrough-token"
...
echo "FinDesk v2 manual responsive walkthrough: setup ${TMP_DIR}"
...
VALUES (19301, 'v2-manual-walkthrough@example.test', 'V2 Manual Walkthrough', 'en', 'UTC', 'active', NOW());

INSERT INTO sessions (user_id, session_token_hash, device_label, ip_address, user_agent, expires_at, last_seen_at)
VALUES (19301, '${TOKEN_HASH}', 'manual-walkthrough', '127.0.0.1', 'FinDesk v2 manual walkthrough', DATE_ADD(NOW(), INTERVAL 1 DAY), NOW());
SQL
...
RESULTS="${ROOT}/test-results/v2-manual-responsive"
rm -rf "${RESULTS}"
mkdir -p "${RESULTS}"
...
FINDESK_V2_BROWSER_SOCKET="${SOCKET}" \
FINDESK_V2_BROWSER_DB="${DB_NAME}" \
FINDESK_V2_BROWSER_RESULTS="${RESULTS}" \
node "${ROOT}/scripts/v2_manual_responsive_walkthrough.cjs"
