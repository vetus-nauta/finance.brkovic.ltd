...
Lower accounting block:

```text
Counterparty | Issued | Returned | Open | Status | Rows
```

Purpose:

```text
show debt/loan/return/accountable/guest-cash control rows outside operational category totals
show who still has open accountable money
show which chains are partial, closed, unresolved, or need review
```

Every settlement row must open the source entries that created that counterparty chain.

Operational entries from the same person, such as normal boat parts, transport, provisions, or representative expenses, must not enter that settlement chain unless the row is classified as lower accounting.
...
status
main totals
money position: physically available total, admin cash, employee-held cash, reimbursement due
cash summary
card summary
category totals
lower accounting
...
The money position block must not treat admin cash as the whole available pool when accountable employees still hold cash. Employee-held cash is included in physically available money; reimbursement due to employees remains a separate obligation.
